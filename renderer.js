...
function updateSidebarVisibility() {
    if (currentView === 'playlists') playlistContainer.style.display = 'flex';

    else playlistContainer.style.display = 'none';
}

function renderCurrentView() {
    if (currentView === 'music') renderMusicLibrary();

    else if (currentView === 'playlist' && currentPlaylistId) renderPlaylistTracks(currentPlaylistId);

    else tracksListEl.innerHTML = '<li style="justify-content: center; color: var(--text-muted);">Выберите плейлист слева</li>';
}
...
async function loadMetadata(filePath) {
    ...
    try {
        ...
        if (result.success) {
            ...
            if (tags.picture) coverImage.src = `data:${tags.picture.format};base64,${tags.picture.data}`;

            else coverImage.src = 'data:image/svg+xml,%3Csvg xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 100 100\' fill=\'%23a0a0a0\'%3E%3Cpath d=\'M70 25v30c0 8-7 15-15 15s-15-7-15-15 7-15 15-15c3 0 6 1 9 2V27l-20 5v30c0 8-7 15-15 15s-15-7-15-15 7-15 15-15c3 0 6 1 9 2V20l30-7v12z\'/%3E%3C/svg%3E';
        } else {
...
function addTrackToPlaylist(playlistId, trackPath, trackName) {
    ...
    if (playlist) {
        if (!playlist.tracks.some(t => t.path === trackPath)) {
            ...
            if (currentPlaylistId === playlistId) renderPlaylistTracks(playlistId);
        } else alert('Трек уже в плейлисте');
    }
}
...
function showPlaylistContextMenu(event, playlistId) {
    ...
    item.addEventListener('mouseleave', () => item.style.background = 'transparent');
    item.addEventListener('click', () => {
        if (confirm(`Удалить плейлист "${playlist.name}"?`)) deletePlaylist(playlistId);
...
function deletePlaylist(playlistId) {
    ...
    if (currentPlaylistId === playlistId) {
        ...
        navItems.forEach(item => {
            if (item.dataset.view === 'music') item.classList.add('active');
...
loopBtn.addEventListener('click', () => {
    ...
    if (loopEnabled) loopBtn.classList.add('active');
...
if (audioFiles.length === 0) renderMusicLibrary();
